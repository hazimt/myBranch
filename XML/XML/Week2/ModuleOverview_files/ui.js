...
ANGEL.sessionTimer = {
    ...
    updateLastAccess: function(dDate) {
        ...
        if (ANGEL.sessionTimer.lastAccess.getTime() - ANGEL.sessionTimer.timerStarted.getTime() > 1 * 60 * 1000) {
    ...
    warn: function() {
        if (ANGEL.user && ANGEL.user.username != '' && ANGEL.config && ANGEL.config.timeout) {
            ...
            ANGEL.sessionTimer.timer = setTimeout(ANGEL.sessionTimer.endSession, ANGEL.sessionTimer.countdown);
        }
    },
